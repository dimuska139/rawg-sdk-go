...
func TestClient_GetGameMovies(t *testing.T) {
	httpmock.Activate()
	...
	items, err := client.GetGameMovies(gameID)
	assert.NoError(t, err)
	assert.Equal(t, 1, len(items))
	assert.Equal(t, "Coming Soon Teaser", items[0].Name)
}

func TestClient_GetGameMovies_HttpError(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
...
func TestClient_GetGameMovies_InvalidJson(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
